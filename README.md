...
(1) movieController.js의 세 함수 (1-1) home
    ...
    DB에서 모든 영화를 찾아 홈 화면으로 보내주기 위해서는 DB에 있는 모든 영화를 배열(array)로 반환하는 함수인 getMovies()를 사용하여
    movies 배열을 정의하고 movies.pug로 pageTitle과 함께 보내면 됩니다.
...
(1-3) filterMovie
    ...
    가져온 값을 getMovieByMinimumYear(year)와 getMovieByMinimumRating(rating) 함수를 사용하여 배열 movies를 정의하고 홈 화면인 movies.pug로
    pageTitle과 함께 보내면 됩니다.
